...
#     st.session_state.local_llm_model = os.getenv("LOCAL_LLM_MODEL")
#     st.session_state.tokenizer = AutoTokenizer.from_pretrained(st.session_state.local_llm_model)
#     st.session_state.model = AutoModelForCausalLM.from_pretrained(st.session_state.local_llm_model)
...
if st.session_state.load:
    ...
    with st.chat_message("assistant"):
        ...
        # data_gen = st.session_state.model.generate(
        #     st.session_state.tokenizer(full_prompt, return_tensors="pt").input_ids,
        #     max_length=100,
        # )
        # data_out = st.session_state.tokenizer.decode(data_gen[0], skip_special_tokens=True)
        data_out = "This is a placeholder response. Replace with actual LLM output."
        response = st.write(data_out)
